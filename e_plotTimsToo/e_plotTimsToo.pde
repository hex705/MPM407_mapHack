...
// add an ANDROID location for Tims -- do not confuse with Ketai.location
// see Example--> locationDistance
...
void setup() {
  ...
  timsDisplay = mercatorMap.getScreenLocation( 43.65838d, -79.37819d );  // got this from google maps
 
  // the tims logo
  timsLogo = loadImage("timCup.jpeg");
  
  // ANDROID locaiton of Tim's
  timsLocation = new Location("timsLocation"); // Example location: the University of Illinois at Chicago
  timsLocation.setLatitude(43.65838);
...
void draw() {
  ...
  // draw a tims cup on Tims -- gps location from google
    pushStyle();
        imageMode(CENTER);
        fill( 255, 20, 0, 100 );
